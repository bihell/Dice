...
import com.bihell.dice.commons.constant.DiceConsts;
import com.bihell.dice.commons.exception.BusinessException;
import com.bihell.dice.commons.service.RedisService;
...
import com.bihell.dice.commons.utils.DiceUtil;
...
public class ArticleServiceImpl extends BaseServiceImpl<ArticleMapper, Article> implements ArticleService {
    ...
    public Integer saveArticle(Article article) {
        if (null == article.getCreator()) {
            throw new BusinessException("请先登陆后发布文章");
        }
    ...
    public boolean updateArticle(Article article) {
        if (null == article) {
            throw new BusinessException("文章不能为空");
        }
     ...
     */
    @Override
    // TODO: 2023/6/26 这里用了 redis 之后缓存序列化有问题
    ...
    public Integer savePage(Article page) {
        if (null == page) {
            throw new BusinessException("自定义页面对象为空");
        }
        if (StringUtils.isEmpty(page.getTitle())) {
            throw new BusinessException("自定义页面标题不能为空");
        }
        if (page.getTitle().length() > DiceConsts.MAX_TITLE_COUNT) {
            throw new BusinessException("自定义页面标题字数不能超过" + DiceConsts.MAX_TITLE_COUNT);
        }

        if (StringUtils.isEmpty(page.getContent())) {
            throw new BusinessException("自定义页面内容不能为空");
        }
        if (page.getContent().length() > DiceConsts.MAX_CONTENT_COUNT) {
            throw new BusinessException("自定义页面容字数不能超过" + DiceConsts.MAX_CONTENT_COUNT);
        }
    ...
    public Integer saveSnippet(Article snippet) {
        if (null == snippet) {
            throw new BusinessException("自定义代码段对象为空");
        }
        if (StringUtils.isEmpty(snippet.getTitle())) {
            throw new BusinessException("自定义代码段标题不能为空");
        }
        if (snippet.getTitle().length() > DiceConsts.MAX_TITLE_COUNT) {
            throw new BusinessException("自定义代码段标题字数不能超过" + DiceConsts.MAX_TITLE_COUNT);
        }

        if (StringUtils.isEmpty(snippet.getContent())) {
            throw new BusinessException("自定义代码段内容不能为空");
        }
        if (snippet.getContent().length() > DiceConsts.MAX_CONTENT_COUNT) {
            throw new BusinessException("自定义代码段容字数不能超过" + DiceConsts.MAX_CONTENT_COUNT);
        }
