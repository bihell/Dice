...
import com.bihell.dice.config.properties.DiceAopProperties;
...
import com.bihell.dice.framework.util.*;
...
public abstract class BaseLogAop {
     ...
     * Aop日志配置
     */
    protected DiceAopProperties.LogAopConfig logAopConfig;

    /**
     * 日志打印类型
     */
    protected DiceAopProperties.LogPrintType logPrintType;
     ...
     * requestId生成类型
     */
    protected DiceAopProperties.RequestIdType requestIdType;

    /**
     * Aop操作日志配置
     */
    protected DiceAopProperties.OperationLogConfig operationLogConfig;

    /**
     * Aop登录日志配置
     */
    protected DiceAopProperties.LoginLogConfig loginLogConfig;

    @Autowired
    public void setDiceAopProperties(DiceAopProperties springBootPlusAopProperties) {
    ...
    protected void handleRequestId(RequestInfo requestInfo) {
        ...
        if (DiceAopProperties.RequestIdType.IDWORK == requestIdType) {
            requestId = IdWorker.getIdStr();
        } else if (DiceAopProperties.RequestIdType.UUID == requestIdType) {
    ...
    protected void handleRequestInfo(RequestInfo requestInfo) {
        ...
        if (DiceAopProperties.LogPrintType.NONE == logPrintType) {
            return;
        }
        // 获取请求信息
        String requestInfoString = formatRequestInfo(requestInfo);
        // 如果打印方式为顺序打印，则直接打印，否则，保存的threadLocal中
        if (DiceAopProperties.LogPrintType.ORDER == logPrintType) {
    ...
    protected void handleResponseResult(Object result) {
        ...
        if (result != null && result instanceof ApiResult) {
            ...
            if (DiceAopProperties.LogPrintType.ORDER == logPrintType) {
                ...
                if (DiceAopProperties.LogPrintType.LINE == logPrintType) {
                    printRequestInfoString(requestInfoString);
                    printResponseResult(code, responseResultString);
                } else if (DiceAopProperties.LogPrintType.MERGE == logPrintType) {
    ...
    protected String formatRequestInfo(RequestInfo requestInfo) {
        ...
        return AnsiOutput.toString(AnsiColor.GREEN, "requestInfo:",requestInfoString);
    }
    ...
    protected String formatResponseResult(ApiResult<?> apiResult) {
        ...
        try {
            ...
            int code = apiResult.getCode();
            if (code == ApiCode.SUCCESS.getCode()) {
                return AnsiOutput.toString(AnsiColor.BLUE, responseResultString);

            } else {
                return AnsiOutput.toString(AnsiColor.RED, responseResultString);
    ...
    protected void saveSysOperationLog(RequestInfo requestInfo, OperationLogInfo operationLogInfo, Object result, Exception exception) {
        try {
            ...
            if (exception != null) {
                ...
                if (exception instanceof DiceException) {
                    DiceException springBootPlusException = (DiceException) exception;
