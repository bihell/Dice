...
public class CacheLockInterceptor {
    ...
    @Around("@annotation(com.bihell.dice.cache.lock.CacheLock)")
    public Object interceptCacheLock(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            ...
            if (cacheResult == null) {
                throw new TipException("Unknown reason of cache " + cacheLockKey);
            }

            if (!cacheResult) {
                throw new TipException("访问过于频繁，请稍后再试！");
            }
    ...
    private String buildCacheLockKey(@NonNull CacheLock cacheLock, @NonNull ProceedingJoinPoint joinPoint) {
        ...
        if (cacheLock.traceRequest()) {
            ...
            cacheKeyBuilder.append(delimiter).append(ServletUtil.getRequestIp());
        }
